...
function getTabs(el: TabContainerElement): HTMLElement[] {
   return Array.from(
      el.querySelectorAll<HTMLElement>('[role="tablist"] [role="tab"]')
   ).filter(tab => tab instanceof HTMLElement && tab.closest(el.tagName) === el);
}

function getNavigationKeyCodes(
   vertical: boolean
): [IncrementKeyCode[], DecrementKeyCode[]] {
   if (vertical) {
      return [
...
export default class TabContainerElement extends HTMLElement {
   constructor() {
      ...
      this.addEventListener("keydown", (event: KeyboardEvent) => {
         ...
         const [incrementKeys, decrementKeys] = getNavigationKeyCodes(
            target.closest('[role="tablist"]')?.getAttribute("aria-orientation") ===
               "vertical"
         );
   ...
   selectTab(index: number): void {
      ...
      ).filter(panel => panel.closest(this.tagName) === this);
