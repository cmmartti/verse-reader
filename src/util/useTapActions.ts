...
export function useTapActions<E extends HTMLElement | null>(
      ...
      function touchEndHandler(event: TouchEvent) {
         ...
         if (event.changedTouches.length === 1) {
            ...
            if (
               !touchEnd ||
               !touchStart ||
               //↓ Touch target can be focused
               (touchEnd.target instanceof HTMLElement &&
                  touchEnd.target.tabIndex >= 0) ||
               //↓ Touch target is a control
               (touchEnd.target instanceof Element &&
                  ["A", "BUTTON", "INPUT", "TEXTAREA"].includes(
                     touchEnd.target.tagName
                  )) ||
